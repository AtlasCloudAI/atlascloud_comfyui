...
class AtlasLtx25ProTextToVideo:
    """
    ComfyUI Node: AtlasCloud LTX 2.5 Pro Text-to-Video (remote API)
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "duration": ([-1, 6, 8, 10], {"default": -1, "tooltip": "Video length in seconds (-1 lets the model choose)"}),
    ...
    def run(
        ...
        if not prompt:
            raise RuntimeError("prompt is required for LTX 2.5 Pro Text-to-Video")

        client = atlas_client.client

        payload: Dict[str, Any] = {
            "model": "ltx/ltx-2.5-pro/text-to-video",
            "prompt": prompt,
