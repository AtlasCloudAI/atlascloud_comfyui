...
class AtlasLtx23ProAudioToVideo:
    """
    ComfyUI Node: AtlasCloud LTX 2.3 Pro Audio-to-Video (remote API)
    ...
    def run(
        ...
        if not audio:
            raise RuntimeError("audio is required for LTX 2.3 Pro Audio-to-Video")

        image = (image or "").strip()
        last_image = (last_image or "").strip()
        prompt = (prompt or "").strip()
        if not image and not prompt:
            raise RuntimeError("prompt is required for LTX 2.3 Pro Audio-to-Video when image is empty")
        if last_image and not image:
            raise RuntimeError("last_image requires image for LTX 2.3 Pro Audio-to-Video")

        client = atlas_client.client

        payload: Dict[str, Any] = {
            "model": "ltx/ltx-2.3-pro/audio-to-video",
            "audio": audio,
